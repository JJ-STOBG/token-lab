...
export function PreviewWorkspace({ resolved, onSelectComponent }: PreviewWorkspaceProps) {
    ...
    return (
        <section className="preview-area" aria-label="Preview workspace">
            ...
            <div className="preview-canvas"><div className="site-preview"><div className="site-nav"><strong>STOBG</strong><span>What we do</span><span>Our work</span><span>About us</span><button>Contact us ↗</button></div>{activePage === 'homepage' && <Homepage resolved={resolved} onSelectComponent={onSelectComponent} />}{activePage === 'markets' && <EditorialPage resolved={resolved} onSelectComponent={onSelectComponent} eyebrow="MARKETS" title="Places with purpose." body="From civic destinations to thriving workplaces, we shape environments that help people and economies move forward." accent="Explore our markets" cards={['Workplace', 'Living', 'Education']} />}{activePage === 'project-detail' && <EditorialPage resolved={resolved} onSelectComponent={onSelectComponent} eyebrow="PROJECT DETAIL" title="North Quay Commons" body="A new neighbourhood for the river edge, bringing work, culture and everyday life closer together." accent="View project story" cards={['The brief', 'The approach', 'The outcome']} />}{activePage === 'careers' && <EditorialPage resolved={resolved} onSelectComponent={onSelectComponent} eyebrow="CAREERS" title="Make your mark here." body="Bring your curiosity, craft and perspective to a team building better places every day." accent="See open roles" cards={['Benefits', 'Our culture', 'Open roles']} />}{activePage === 'contact' && <Contact resolved={resolved} onSelectComponent={onSelectComponent} />}</div></div>
        </section>
    )
}
...
function EditorialPage({ resolved, onSelectComponent, eyebrow, title, body, accent, cards }: PreviewWorkspaceProps & { eyebrow: string; title: string; body: string; accent: string; cards: string[] }) {
    return <><button className="editorial-hero" onClick={() => onSelectComponent('homepage-hero')} style={{ backgroundColor: resolved['accent-brand'].value }}><span className="preview-tag">{eyebrow}</span><h3>{title}</h3><p>{body}</p><span className="hero-cta" style={{ backgroundColor: resolved['action-primary'].value, color: resolved['text-primary'].value }}>{accent} <b>↗</b></span></button><div className="editorial-grid" style={{ backgroundColor: resolved['background-subtle'].value }}>{cards.map((card, index) => <div className="editorial-card" key={card}><span>0{index + 1}</span><strong style={{ color: resolved['text-primary'].value }}>{card}</strong><p>Thoughtful detail, clear intent and a lasting impact.</p></div>)}</div></>
}
